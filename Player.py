...
class Player(Sprite):
    ...
    def update(self, sprites, screen, friends, boom_group, player_spavn):
        ...
        if self.timer == 34:
            self.timer = 0
            self.ready = True
            self.recharge = Surface((34, 5))
            self.recharge.fill((0, 250, 0))
        self.yvel = self.xvel = 0
        if self.dir == 'left':
            self.xvel = -self.MOVE_SPEED
            self.boltAnimLeft.blit(self._image, (0, 0))
            self.image = scale(self._image, (34, 34))
        elif self.dir == 'right':
            self.xvel = self.MOVE_SPEED
            self.boltAnimRight.blit(self._image, (0, 0))
            self.image = scale(self._image, (34, 34))
        elif self.dir == 'up':
            self.yvel = -self.MOVE_SPEED
            self.boltAnimUp.blit(self._image, (0, 0))
            self.image = scale(self._image, (34, 34))
        elif self.dir == 'down':
            self.yvel = self.MOVE_SPEED
            self.boltAnimDown.blit(self._image, (0, 0))
            self.image = scale(self._image, (34, 34))
        elif self.ldir == 'left' and self.dir == '':
            self.xvel = 0
            self.yvel = 0       
            self.boltAnimStayLeft.blit(self._image, (0, 0))
            self.image = scale(self._image, (34, 34))
        elif self.ldir == 'right' and self.dir == '':
            self.xvel = 0
            self.yvel = 0
            self.boltAnimStayRight.blit(self._image, (0, 0))
            self.image = scale(self._image, (34, 34))
        elif self.ldir == 'down' and self.dir == '':
            self.yvel = 0
            self.xvel = 0
            self.boltAnimStayDown.blit(self._image, (0, 0))
            self.image = scale(self._image, (34, 34))
        if self.ldir == 'up' and self.dir == '':
            self.yvel = 0
            self.xvel = 0
            self.boltAnimStayUp.blit(self._image, (0, 0))
            self.image = scale(self._image, (34, 34))
    ...
    def collide_crutch(self, sprites):
        for pl in sprites:
            if collide_rect(self, pl) and pl != self:
                if self.yvel > 0:
                    self.rect.bottom = pl.rect.top
